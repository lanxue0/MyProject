...
export function TeacherJoinClassroom() {
  ...
  const fetchClassrooms = async () => {
    setLoading(true);
    try {
      const res = await getTeachingClassrooms();
      if (res.status === 200) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (e.response?.status === 400) {
        if (e.response?.data?.message.includes("创建者")) {
          messageApi?.warning("您已是该班级的创建者");
        } else {
          messageApi?.warning("您已加入该班级");
        }
      } else {
        messageApi?.error(e.response?.data?.message || "加入班级失败");
      }
    }
  };
  ...
  const handleLeave = async (classroomId: number) => {
    try {
      const res = await leaveTeacherClassroom(classroomId);
      if (res.status === 200) {
  ...
  return (
    <div>
      <Card
        title={<Title level={4}>协作班级</Title>}
        extra={
          <Space>
            <Input
      ...
      >
        {foundClassroom && (
          <div>
            ...
            <p>
              <strong>创建教师:</strong> {foundClassroom.creator?.username}
            </p>
            <p>
      ...
      >
        {selectedClassroom && (
          <Space direction="vertical" style={{ width: "100%" }}>
            ...
            <Space direction="vertical" style={{ width: "100%" }}>
              <p>
                <strong>创建教师:</strong> {selectedClassroom.creator?.username}
              </p>
              <p>
                <strong>学生数量:</strong> {selectedClassroom.studentCount}
              </p>
              <p>
                <strong>加入时间:</strong>{" "}
            ...
            <div style={{ textAlign: "right", marginTop: 16 }}>
              <Popconfirm
                title="确定要退出该班级吗？"
                onConfirm={() => handleLeave(selectedClassroom.id)}
                okText="确定"
                cancelText="取消"
              >
